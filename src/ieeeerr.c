...
int setfpumask(int fpmask) {
  ...
  fpucw = (fpucw & 0xffc0) | (fpmask & FE_ALL_EXCEPT);
  ...
  mxcsr = (mxcsr & 0xffffe07f) | ((fpmask & FE_ALL_EXCEPT) << 7);
...
int setfpumask(int fpmask) {
  ...
  oldmask = cw & ~0xffc0;
  cw = (cw & 0xffc0) | (fpmask & FE_ALL_EXCEPT);
   ...
   void *where;
{
   ...
   fprintf(stderr,"IEEE error %x at %p: %s.\n",
      code,where,words);
   ...
   char *what;
{
...
#elif !defined (__ppc__) && defined (__APPLE__)
   {
     ...
     if (strstr(ptr,"invalid")) mask |= FE_INVALID ;
