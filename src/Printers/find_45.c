...
int code;
{
	...
	for (type45 = type45_head; type45 != NULL; type45 = type45->next)
	{
		if (type45->response_code == code)
		{
			...
			fprintf (fp,"%s  i, frequency,  amplitude,    amplitude error, phase,        phase error\n",
					 com_strt);
...
int code;
{
	...
	for (type45 = type45_head; type45 != NULL; type45 = type45->next)
	{
		if (type45->response_code == code)
		{
			...
			fprintf (fp,"  i, frequency,   amplitude,    amplitude error, phase,        phase error\n");

			for (i = 0; i < type45->number_responses; i++)
				fprintf (fp,"%3d %E %E %E    %E %E\n", i, type45->response[i].frequency, 
